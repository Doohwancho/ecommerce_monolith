...
public class InsertFakeUsersStepConfig {
    ...
    public ItemReader<UserEntity> generateRandomROLE_USERReader(
        @Value("#{jobParameters['numberOfFakeUsers']}") Long numberOfFakeUsers) {
...
//        attribute.setTimeout(30); // 30 seconds
        ...
        return stepBuilderFactory.get("insertFakeUserStep")
            .<UserEntity, UserEntity>chunk(
                1000) //<UserEntity, UserEntity>에서 첫번째 인자는 .reader()가 리턴하는 인자이고, 두번째 인자는 writer()가 리턴하는 인자이다.
            .reader(
                generateRandomROLE_USERReader) //Spring Batch manages transactions at the chunk level
