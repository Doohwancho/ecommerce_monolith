...
public class ProductPriceDiscountTest {
    ...
    Arbitrary<OffsetDateTime> offsetDateTimes() {
        ...
        return DateTimes.offsetDateTimes().atTheEarliest(now.minusYears(1).toLocalDateTime())
            .atTheLatest(now.plusYears(1).toLocalDateTime()); //오늘 기준, 과거 -1년 ~ 미래 1년 사이 랜덤한 시간 선정함.
    }
    
    @Property
    void getPERCENTAGEDiscountedPriceOfProduct(@ForAll @DoubleRange(min = 0.0) Double originalPrice,
        ...
        for (int i = 0; i < 3; i++) {
            Discount discount = new Discount.Builder().discountType(DiscountType.PERCENTAGE)
                .discountValue(discountValue).endDate(endDate).build();
            discountList.add(discount);
        }
        
        Product product = new Product.Builder().price(originalPrice).discounts(discountList)
            //rest of properties are not necessary for this test, but fill it to avoid exception handling
            .quantity(0).name("").categoryId(0L).categoryName("").categoryCode("").optionName("")
            .build();
        ...
        Double targetDiscountedPrice = discountList.stream()
            .reduce(originalPrice, (price, discount) -> discount.applyDiscount(price),
    ...
    @Property
    void getFLATRATEDiscountedPriceOfProduct(@ForAll @DoubleRange(min = 0.0) Double originalPrice,
