package com.ecommerce.monolith.domain.product.service;
...
import com.ecommerce.monolith.domain.product.entity.DenormalizedProductEntity;
import com.ecommerce.monolith.domain.product.mapper.ProductMapper;
import com.ecommerce.monolith.domain.product.repository.ProductRepository;
...
public class ProductService {
    ...
    // 상품 재고를 다시 늘리는 보상 트랜잭션용 메서드
    @Transactional
    public void increaseStock(String productName, int quantity) {
        DenormalizedProductEntity productEntity = productRepository.findProductsByName(productName);
    
        if (productEntity != null) {
            productEntity.setTotalQuantity(productEntity.getTotalQuantity() + quantity);
        
            // @Transactional 어노테이션으로 인해 'dirty checking'이 동작하므로
            // 명시적인 save 호출이 없어도 DB에 반영되지만, 가독성을 위해 save를 호출하는 것도 좋은 방법입니다.
            productRepository.save(productEntity);
        } else {
            // 상품을 찾을 수 없는 경우에 대한 예외 처리
            throw new RuntimeException("Product " + productName + " not found for restocking.");
        }
    }
}