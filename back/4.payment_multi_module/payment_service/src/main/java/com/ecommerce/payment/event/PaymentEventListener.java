...
public class PaymentEventListener {
    ...
    public void handlePaymentFailed(PaymentFailedEvent event) {
        // 후속 처리 로직
        log.warn("Payment failed! Notifying other services for orderId: {}", event.getPayment().getOrderId());
        ...
        String failureMessage = event.getPayment().getFailureMessage();
    ...
    public void handlePaymentCancelled(PaymentCancelledEvent event) {
        log.warn("Payment cancelled! Notifying other services for orderId: {}", event.getPayment().getOrderId());
